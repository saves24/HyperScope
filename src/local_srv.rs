...
fn handle_control(req: &Value) -> Value {
    ...
    let authed = if !device_id.is_empty() && !signature.is_empty() {
        ...
        match hyper_panel_core::identity::find_device(&list, device_id) {
            Some(dev) => {
                ...
                let msg = format!("{raw_action}:{device_id}");
                hyper_panel_core::identity::verify_device_signature(&dev.pubkey, &msg, signature)
            }
            None => false,
        }
    } else {
    ...
    // High-risk actions (reboot/shutdown) require admin+ role for signed callers.
    if matches!(action.as_str(), "reboot" | "shutdown") {
        let list = hyper_panel_core::identity::load_trusted();
        let role_ok = if device_id.is_empty() {
            true // local root caller
        } else {
            hyper_panel_core::identity::find_device(&list, device_id)
            ...
            match parts.as_slice() {
                ...
                ["docker", container, action] => {
                    ...
                    if !matches!(*action, "start" | "stop" | "restart") {
                        return json!({"type": "result", "ok": false, "error": "bad docker action"});
                    }
