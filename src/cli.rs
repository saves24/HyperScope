use crate::{
    ...
    set_retention_days, CERT_FILE, DEFAULT_RETENTION_DAYS, KEY_FILE, LOG_DIR, VERSION,
};
use std::fs;
...
Commands:
  ...
  cert import       import a shared certificate: cert import <cert.pem> <key.pem>
  cert show         show current certificate SHA256 fingerprint
  identity init     generate the Ed25519 identity key (prints public key)
  identity show     show the identity public key
  identity sign <msg>  sign a message with the identity key
  device list       list trusted devices
  device add <id> <pubkey> <role>  trust a device (owner|admin|viewer)
  device remove <id>  remove a trusted device
  trust add <fingerprint>  trust a panel client certificate fingerprint (mTLS)
  trust list        list all trusted certificate fingerprints
  trust clear       clear all trusted certificate fingerprints
  collect           print the full system snapshot as JSON (one-shot)
  control <path> [device_id] [signature]
                    run a control action (reboot/shutdown/process kill/docker);
                    device signature is verified when provided (relay path)
  relay | serve     run in relay mode (no listening port; woken by hyper-relay)
...
Config:
  ...
  cert:      {CERT_FILE} / {KEY_FILE} (self-signed, auto-generated)
  trusted:   /etc/hyper-node/trusted.toml (device identity trust)
