...
pub(crate) fn get_system() -> Value {
    ...
    let traffic = platform::get_traffic(None);
    #[cfg(target_os = "windows")]
    let io = platform::get_io();
    #[cfg(not(target_os = "windows"))]
    let io = json!({});
    // Include the process list and docker containers in the snapshot so the
    // panel can serve them from the relay cache (no direct HTTP path).
    let processes_list = platform::get_processes("mem", 50, "");
    let docker = platform::get_docker_containers();
