...
pub(crate) fn get_processes(sort: &str, limit: usize, name_filter: &str) -> Vec<Value> {
    ...
    for e in dir.flatten() {
        ...
        // Comm from /proc/pid/stat can be truncated or contain parentheses
        // (e.g. "next-server (v"), which breaks a simple split on '('.
        // Prefer the full command line; fall back to the stat comm. Show the
        // basename only so the panel rows stay compact (full paths would wrap).
        let mut comm = read_file(&format!("/proc/{pid}/cmdline"))
            .map(|c| {
                c.split('\0')
                    .next()
                    .map(|s| s.trim().rsplit('/').next().unwrap_or_default().to_string())
                    .unwrap_or_default()
            })
            .unwrap_or_default();
        if comm.is_empty() {
        ...
        // Strip a trailing "(<arg>)" suffix some kernels append to the comm.
        if let Some((base, _)) = comm.rsplit_once(" (") {
            comm = base.to_string();
        }
    ...
    sorted
        ...
        .map(|(pid, comm, state, _, rss_kb)| {
            json!({
                ...
                "rss": rss_kb,
                "rss_mb": (*rss_kb as f64 / 1024.0 * 10.0).round() / 10.0,
            })
        })
        .collect()
}
...
pub(crate) fn kill_process(pid: u32) -> std::io::Result<()> {
    // SIGTERM first (graceful), fall back to SIGKILL if the process ignores it.
    unsafe {
...
/// Docker container list via the docker CLI (same shape as Windows).
...
/// Docker container control via the docker CLI (start/stop/restart).
pub(crate) fn docker_action(name: &str, action: &str) -> Value {
    let docker_action = if action == "delete" { "rm" } else { action };
    let out = std::process::Command::new("docker")
        .args([docker_action, name])
        .output();
    match out {
        Ok(o) => {
...
/// Returns the last `lines` of a container's logs (docker logs --tail).
pub(crate) fn docker_logs(name: &str, lines: u32) -> Value {
    let out = std::process::Command::new("docker")
    ...
    match out {
        Ok(o) => json!({
            "ok": o.status.success(),
            "logs": String::from_utf8_lossy(&o.stdout).trim().to_string(),
        }),
        Err(e) => json!({"ok": false, "error": e.to_string()}),
    }
}
