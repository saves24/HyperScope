...
// Compute certificate SHA256 fingerprint (format: SHA256:xxxx)
...
#[cfg(target_os = "windows")]
pub(crate) const HOSTNAME_FILE: &str = "C:\\Windows\\Temp\\hostname.txt";

// ---------- Basic utilities ----------

// ---------- Logging (daily rotation + periodic cleanup) ----------

// Days to Gregorian calendar (Howard Hinnant algorithm)
        ...
        Some("mode") => {
            match args.get(1).map(|s| s.as_str()) {
                Some("tls") | Some("plain") => {
                    ...
                    if let Err(e) = std::fs::write(MODE_FILE, mode.clone()) {
                        ...
                        #[cfg(target_os = "linux")]
                        {
                            ...
                            if std::path::Path::new(service).exists() {
                                ...
                                if let Err(e) = std::fs::write(service, updated) {
