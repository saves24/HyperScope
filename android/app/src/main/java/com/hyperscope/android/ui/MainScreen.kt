...
fun MainScreen(vm: AppViewModel) {
    ...
    // Initial tab comes from an app shortcut (VM state), read once here.
    var tabName by rememberSaveable { mutableStateOf(vm.initialTab.value) }
    // Safe parse: initialTab may be "dashboard" (lowercase) or any stale value;
    // Tab.valueOf would throw and crash the whole screen on login. Fall back
    // to the Dashboard tab.
    val tab = Tab.entries.firstOrNull { it.name.equals(tabName, ignoreCase = true) } ?: Tab.Dashboard
