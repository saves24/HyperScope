 ...
 * hyper-relay (WSS, TLS pinned to the node's certificate fingerprint) for a
 * fresh metrics snapshot. No direct HTTP port is ever opened by the collector.
 */
object RelayClient {
     ...
     */
    suspend fun queryCollect(relayHost: String, nodeName: String, tls: Boolean = true, certFp: String = ""): JSONObject? =
        withContext(Dispatchers.IO) {
            try {
                val ws = WebSocketClient("wss://$relayHost:$RELAY_PORT/ws", true, certFp)
                ws.connect(5000)
     ...
     */
    suspend fun sendCmd(relayHost: String, nodeName: String, cmd: String, tls: Boolean = true, auth: Pair<String, String> = "" to "", certFp: String = ""): Boolean =
        withContext(Dispatchers.IO) {
            try {
                val ws = WebSocketClient("wss://$relayHost:$RELAY_PORT/ws", true, certFp)
                ws.connect(5000)
...
class WebSocketClient(private val url: String, private val useTls: Boolean = false, private val expectedFp: String = "") {
    ...
    fun connect(timeoutMs: Int) {
        ...
        val s: Socket = if (useTls) {
            // Pin the relay's self-signed certificate by SHA-256 fingerprint
            // (same trust model as the HTTP client). When no expected
            // fingerprint is configured, accept the first certificate and
            // record it (TOFU) — never trust-all blindly.
            val ctx = javax.net.ssl.SSLContext.getInstance("TLS")
            val trust = object : javax.net.ssl.X509TrustManager {
                override fun checkClientTrusted(chain: Array<out java.security.cert.X509Certificate>?, authType: String?) {}
                override fun checkServerTrusted(chain: Array<out java.security.cert.X509Certificate>?, authType: String?) {
                    if (chain == null || chain.isEmpty()) throw java.security.cert.CertificateException("no server cert")
                    val fp = sha256Fp(chain[0])
                    if (expectedFp.isNotEmpty() && fp != expectedFp) {
                        throw java.security.cert.CertificateException("cert fp mismatch: got $fp expected $expectedFp")
                    }
                }
                override fun getAcceptedIssuers(): Array<java.security.cert.X509Certificate> = arrayOf()
            }
            ctx.init(null, arrayOf<javax.net.ssl.X509TrustManager>(trust), java.security.SecureRandom())
...
/** SHA-256 hex fingerprint of a certificate (lowercase, no colons). */
private fun sha256Fp(cert: java.security.cert.X509Certificate): String {
