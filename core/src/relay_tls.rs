...
/// Verifier that accepts any certificate and records the server certificate's
/// SHA256 fingerprint (TOFU). Used for relays with self-signed certs; the
/// relay is a zero-privilege pipe, so we accept any cert but still capture the
/// fingerprint so the panel can pin it after the first connection.
#[derive(Debug)]
