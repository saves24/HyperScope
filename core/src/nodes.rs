...
/// Records an admin action (who did what) into the shared event stream with
/// kind="admin_action". Used for audit: node removed, password changed, etc.
pub async fn record_admin_action(app: &SharedState, actor: &str, msg: String) {
    let mut events = app.events.lock().await;
    let ev = json!({
        "time": format_time(now_unix()),
        "node": "",
...
// SSRF guard: block loopback, link-local metadata and other non-routable
// addresses; private nets (10/172.16/192.168) are allowed
...
pub fn save_nodes(nodes: &[NodeConfig]) -> Result<(), String> {
    ...
    // nodes.json contains node keys: atomic write + 640 (owner rw, group r).
    // CLI runs as root while the panel service runs as its own user, so also
    // chown to the directory owner/group — otherwise the panel cannot read
    // the file it was just told to load (silent 0 nodes).
    crate::atomic_write(CONFIG_FILE, &content, 0o640)?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::{MetadataExt, PermissionsExt};
        if let Ok(meta) = fs::metadata(dir) {
            let _ = fs::set_permissions(CONFIG_FILE, fs::Permissions::from_mode(0o640));
            let _ = std::os::unix::fs::chown(CONFIG_FILE, Some(meta.uid()), Some(meta.gid()));
        }
    }
    Ok(())
}
