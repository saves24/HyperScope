...
pub fn log_write(level: &str, msg: &str) {
    if let Err(error) = fs::create_dir_all(LOG_DIR) {
        eprintln!("log directory unavailable: {error}");
        return;
    }
    ...
    if let Ok(mut f) = fs::OpenOptions::new().create(true).append(true).open(&path) {
        ...
        eprintln!("log file unavailable: {path}");
    }
    cleanup_logs();
}
...
pub fn set_retention_days(days: u64) -> Result<(), String> {
    let conf_dir = std::path::Path::new(LOG_CONFIG)
        .parent()
        .unwrap_or(std::path::Path::new("/etc/hyper-panel"));
    let _ = fs::create_dir_all(conf_dir);
    crate::atomic_write(LOG_CONFIG, &format!("{days}\n"), 0o600)
}
