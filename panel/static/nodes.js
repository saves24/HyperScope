...
function saveNodeAlerts() {
  ...
  if (i < 0) return;
  const body = collectAlertForm();
  apiFetch(nodeApiUrl(i, "/alerts"), { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
      .then(r => r.json())
      .then(d => {
          if (d.ok) { showToast(t("node-alert-saved")); closeAlertEditModal(); loadNodeList(); }
          else showToast(d.error || t("op-err"));
      })
      .catch(() => showToast(t("op-err")));
}
function testNodeAlert() {
  const i = alertEditIdx;
  if (i < 0) return;
  apiFetch(nodeApiUrl(i, "/alerts/test"), { method: "POST" })
      .then(r => r.json())
      .then(d => {
          if (d.ok) showToast(t("node-alert-test-sent"));
          else showToast(d.error || t("op-err"));
      })
      .catch(() => showToast(t("op-err")));
}
  ...
  apiFetch("/api/nodes")
      ...
      .then(d => {
          ...
          if (list.length !== nodes.length || list.some((n, i) => displayName(n) !== nodeNames[i])) {
              ...
              nodeAlertActive = list.map(n => n.alerts || []);
  ...
  grid.innerHTML = nodes.map((name, i) => {
      ...
      <div class="node-card ${i === activeNode ? "active" : ""}" id="nodeCard_${i}" onclick="selectNode(${i})">
          <div class="node-top">
              ...
              ${nodeAlertActive[i] && nodeAlertActive[i].length ? `<span class="node-alert-badge" title="${escapeHtml(nodeAlertActive[i].join(', '))}">${nodeAlertActive[i].length}</span>` : ""}
