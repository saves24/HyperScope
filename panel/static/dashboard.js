...
function updateDiskOverview() {
  ...
  apiFetch(nodeApiUrl(activeNode, "/disks"))
      ...
      .then(d => {
          ...
          if (!body) return;
          if (!d.disks || !d.disks.length) { body.innerHTML = "<div class='empty-hint'>--</div>"; return; }
          body.innerHTML = d.disks.map(x => `
...
function drawTempTrend() {
  ...
  if (!canvas) return;
  const ctx = canvas.getContext("2d");
  const w = canvas.clientWidth || 300, h = canvas.clientHeight || 50;
  canvas.width = w * 2;
  canvas.height = h * 2;
  ctx.scale(2, 2);
  ctx.clearRect(0, 0, w, h);
...
function updateTopProcs() {
  ...
  apiFetch(nodeApiUrl(activeNode, "/processes?sort=cpu&limit=5"))
      ...
      .then(d => {
          ...
          if (!d.processes || !d.processes.length) { body.innerHTML = "<div class='empty-hint'>--</div>"; return; }
