use crate::{
    hash_password, port_in_use, save_users, validate_user_input, User, Value, AUTH_FILE,
    DEFAULT_PORT, SETTINGS_FILE,
};
use serde_json::json;
...
// Whether node is visible to current user (admin sees all; regular users only their own)

// Current user: GET /api/me -> {user, is_admin}

// User list: GET /api/users -> {users: [{name, is_admin}]}

// Add user: POST /api/users {user, pass}

// Update user: PUT /api/users/:name {pass?, new_name?}

// Delete user: DELETE /api/users/:name (cannot delete admin or the last user)
