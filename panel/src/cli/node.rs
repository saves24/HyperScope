...
Commands:
  node add <address> <key>              add node (default port 8686; batch: {{addr key}}{{addr key}}...; --tls enable encrypted connection)
  ...
  identity show                         show panel Ed25519 identity public key (for node device trust)
  setup [--user <username>]            create/reset the single admin account (default admin, interactive password)
  user passwd <username>               change the admin password (interactive)
...
// Parse "host:port" (default port 8686); handles IPv4, bare IPv6, and [IPv6]:port
pub(crate) fn parse_node_address(input: &str) -> Option<(String, u16)> {
    ...
    if input.matches(':').count() > 1 {
        // Bare IPv6 (no port)
        return Some((input.to_string(), 8686));
    }
    match input.split_once(':') {
        ...
        None => Some((input.to_string(), 8686)),
    }
}
