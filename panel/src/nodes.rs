...
pub(crate) async fn nodes_handler(
    ...
    let list: Vec<Value> = nodes
        ...
        .map(|(_, ns)| {
            ...
            // Current raised alerts for this node (from the alert engine's
            // active set) — shown as a badge on the node card. try_lock is
            // used because this map closure cannot await.
            let alerts = app
                .active_alerts
                .try_lock()
                .ok()
                .and_then(|active| active.get(&ns.config.id).cloned())
                .unwrap_or_default();
            json!({
                "id": ns.config.id,
                "name": ns.config.name,
...
pub(crate) async fn test_alert_handler(
    State(app): State<SharedState>,
    headers: HeaderMap,
    axum::extract::Path(ident): axum::extract::Path<String>,
) -> impl IntoResponse {
    // Sends a test notification through the node's configured webhook.
    let user = current_user(&headers);
    let nodes = app.nodes.lock().await;
    let admin = is_admin(&app, &user).await;
    ...
    let url = hyper_panel_core::webhook::webhook_url_for(&app, &ns.config);
    if url.is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"ok": false, "error": "no webhook configured"})),
        )
            .into_response();
    }
    let payload = hyper_panel_core::webhook::alert_payload(
        &ns.config.name,
        "test",
        "HyperScope test notification",
    );
    hyper_panel_core::webhook::spawn_webhook(&url, payload);
    (
        StatusCode::OK,
        Json(json!({"ok": true, "msg": "test notification sent"})),
    )
        .into_response()
}
    ...
    match visible_node_by_ident(app, headers, ident).await {
        Some(ns) => {
            ...
            let (device_id, signature) = match hyper_panel_core::identity::ensure_identity() {
                Ok(pubkey) => {
                    // Sign cmd:device_id:timestamp:nonce so a captured command
                    // cannot be replayed later (relay/collector check the
                    // timestamp window and reject reused nonces).
