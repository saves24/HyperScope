use crate::{
    current_user, fetch_json, http_request_json, is_admin, log_write, node_visible, save_nodes,
    ssrf_blocked, Duration, HttpOptions, JsonExtract, NodeConfig, NodeState, SharedState,
    StatusCode, EVENTS_FILE,
};
...
pub(crate) async fn add_node_handler(
    ...
    // SSRF guard: block link-local metadata + loopback + panel itself (all other private nets allowed)
    if ssrf_blocked(&cfg.addr) {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "address not allowed (link-local / loopback)"})),
        )
            .into_response();
    }
...
pub(crate) async fn events_handler(
    ...
    let user = current_user(&headers);
    let admin = is_admin(&app, &user).await;
    // Node names visible to regular users (admin gets no filter)
