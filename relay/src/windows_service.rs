...
fn service_main_inner() -> Result<(), String> {
    ...
    // cert is missing the service falls back to plain WS so a bad install
    // still leaves the relay usable.
    let cert = "C:\\ProgramData\\hyper-node\\tls\\relay-cert.pem";
    let key = "C:\\ProgramData\\hyper-node\\tls\\relay-key.pem";
    let (cert, key) = if std::path::Path::new(cert).exists() && std::path::Path::new(key).exists() {
        (Some(cert), Some(key))
    } else {
        (None, None)
    };
    ...
    rt.block_on(async {
        if let Err(e) = crate::server::run_with_tls(addr, cert, key).await {
